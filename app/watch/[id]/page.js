...
export default function WatchPage({ params }) {
  ...
  const logoSrc = channel?.logo && !channel.logo.startsWith('data:image/jpeg;base64') && !imgError
    ? channel.logo : null;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0f] transition-colors duration-300">
      <div className="max-w-[1200px] mx-auto px-6 py-6 pb-16">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-2 text-[14px] font-semibold mb-5 px-4 py-2 rounded-xl bg-white dark:bg-[#15151f] border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:text-orange-500 hover:border-orange-500 dark:hover:text-orange-500 dark:hover:border-orange-500 transition-all duration-200 shadow-sm"
        >
        ...
        {channel && (
          <div
            className="animate-fade-in-up mt-6 p-6 rounded-[20px] bg-white dark:bg-[#15151f] border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-5 transition-colors duration-300"
          ...
          >
            ...
            <div className="w-20 h-20 rounded-2xl shrink-0 bg-slate-50 dark:bg-[#0a0a0f] border border-slate-200 dark:border-slate-800 flex items-center justify-center overflow-hidden">
              {logoSrc ? (
                ...
                <span className="text-[28px] font-black text-transparent bg-clip-text" style={{ backgroundImage: 'var(--accent-gradient)' }}>
...
function RelatedChannelCard({ channel, index }) {
  ...
  const logoSrc = channel.logo && !channel.logo.startsWith('data:image/jpeg;base64') && !imgErr
    ? channel.logo : null;
  ...
  return (
    ...
    >
      <div className="h-[100px] flex items-center justify-center bg-slate-50 dark:bg-[#0a0a0f] relative p-3">
        {logoSrc ? (
          <img
            src={logoSrc} alt={channel.name} onError={() => setImgErr(true)}
            className={`w-full h-full object-contain transition-transform duration-500 ${hovered ? 'scale-110' : 'scale-100'}`}
            loading="lazy"
          />
        ) : (
          <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white text-[20px] font-black" style={{ background: 'var(--accent-gradient)' }}>
            {channel.name.replace(/^(IN:|PK:)\s*/i, '').charAt(0)}
          </div>
        )}
